...
//                if(value == "~DELETED~") return "";
//                return value;
                if(sstable_time_tag.first.first >= time_stamp_max){
...
void KVStore::compaction(uint64_t level) {
    ...
    //读取数据
    auto *data = new map<uint64_t,pair<string,uint64_t>>(); //key,<value,time_stamp>
    //LEVEL X
    if(level_config.find(level)->second.second == Tiering){
        string dir_name = "./data/level-" + to_string(level);
        vector<string> file_list;
        ...
        if(level_config.find(level)->second.second == Leveling){
            ...
            for(auto &time_tag_sstable : sstables.find(level+1)->second){
                ...
                if(tmp->get_max_key() >= min_key && tmp->get_min_key() <= max_key){
                    string file_path = "./data/level-" + to_string(level+1) + "/" + to_string(time_tag_sstable.first.first) + "_" + to_string(time_tag_sstable.first.second) + ".ssh";
                    read_data(file_path,data,isLastLevel);
//                    delete_sstables_file(file_path);
    ...
    while(iter != data->end()){
        ...
        //mem_table size > 2MB
        string dir_path = "./data/level-" + to_string(level+1);
...
void KVStore::read_data(const string& file_path, map<uint64_t, pair<string,uint64_t>> *data, bool isLast) {
    ...
    SStable *sstable = this->sstables.find(level)->second.find(make_pair(time_stamp_cur,tag))->second;
...
//        }
        if(!data->empty()){
            auto iter = data->find(key);
            if(iter != data->end()){
                if(time_stamp_cur > iter->second.second){
                    data->erase(key);
                }
            }
        }
        ...
        if(!(isLast && value == "~DELETED~")){
            data->insert(make_pair(key, make_pair(value,time_stamp_cur)));
        }
